  ...
  mycobot_pro450_interfaces__msg__MycobotSetCoords__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mycobot_pro450_interfaces__msg__MycobotSetCoords * data =
      (mycobot_pro450_interfaces__msg__MycobotSetCoords *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mycobot_pro450_interfaces__msg__MycobotSetCoords__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mycobot_pro450_interfaces__msg__MycobotSetCoords__fini(&data[i]);
        }
        free(data);
        return false;
      }
