  ...
  mypalletizer_interfaces__msg__MypalAngles__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mypalletizer_interfaces__msg__MypalAngles * data =
      (mypalletizer_interfaces__msg__MypalAngles *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mypalletizer_interfaces__msg__MypalAngles__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mypalletizer_interfaces__msg__MypalAngles__fini(&data[i]);
        }
        free(data);
        return false;
      }
