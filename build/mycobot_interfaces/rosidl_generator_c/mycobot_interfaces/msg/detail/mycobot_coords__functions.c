  ...
  mycobot_interfaces__msg__MycobotCoords__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mycobot_interfaces__msg__MycobotCoords * data =
      (mycobot_interfaces__msg__MycobotCoords *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mycobot_interfaces__msg__MycobotCoords__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mycobot_interfaces__msg__MycobotCoords__fini(&data[i]);
        }
        free(data);
        return false;
      }
