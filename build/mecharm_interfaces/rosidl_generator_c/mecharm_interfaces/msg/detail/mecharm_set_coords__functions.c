  ...
  mecharm_interfaces__msg__MecharmSetCoords__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mecharm_interfaces__msg__MecharmSetCoords * data =
      (mecharm_interfaces__msg__MecharmSetCoords *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mecharm_interfaces__msg__MecharmSetCoords__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mecharm_interfaces__msg__MecharmSetCoords__fini(&data[i]);
        }
        free(data);
        return false;
      }
