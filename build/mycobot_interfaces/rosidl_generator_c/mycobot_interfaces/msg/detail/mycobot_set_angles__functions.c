  ...
  mycobot_interfaces__msg__MycobotSetAngles__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mycobot_interfaces__msg__MycobotSetAngles * data =
      (mycobot_interfaces__msg__MycobotSetAngles *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mycobot_interfaces__msg__MycobotSetAngles__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mycobot_interfaces__msg__MycobotSetAngles__fini(&data[i]);
        }
        free(data);
        return false;
      }
