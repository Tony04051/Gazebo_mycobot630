  ...
  mecharm_interfaces__msg__MecharmPumpStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mecharm_interfaces__msg__MecharmPumpStatus * data =
      (mecharm_interfaces__msg__MecharmPumpStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mecharm_interfaces__msg__MecharmPumpStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mecharm_interfaces__msg__MecharmPumpStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
