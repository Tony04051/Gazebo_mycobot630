  ...
  mypalletizer_interfaces__msg__MypalPumpStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mypalletizer_interfaces__msg__MypalPumpStatus * data =
      (mypalletizer_interfaces__msg__MypalPumpStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mypalletizer_interfaces__msg__MypalPumpStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mypalletizer_interfaces__msg__MypalPumpStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
